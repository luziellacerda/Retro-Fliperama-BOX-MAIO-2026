...
unpack() {
  ...
  tar --strip-components=1 \
      -xf ${SOURCES}/ffmpeg/ffmpeg-${PKG_VERSION}.tar.xz \
      -C ${PKG_BUILD}
}
...
pre_configure_target() {
  ...
  if [ ${DISTRO} == "EmuELEC" ]; then
    sed -i "s|int hide_banner = 0|int hide_banner = 1|" \
      ${PKG_BUILD}/fftools/cmdutils.c

    sed -i \
      "s|SDL2_CONFIG=\"\${cross_prefix}sdl2-config\"|SDL2_CONFIG=\"${SYSROOT_PREFIX}/usr/bin/sdl2-config\"|" \
      ${PKG_BUILD}/configure
  fi
}
...
configure_target() {
  ...
  ./configure \
      ...
      --extra-cflags="${CFLAGS} -I${SYSROOT_PREFIX}/usr/include/SDL2 -I${SYSROOT_PREFIX}/usr/include" \
      --extra-ldflags="${LDFLAGS}" \
      --extra-libs="${PKG_FFMPEG_LIBS} -lSDL2" \
      --disable-static \
      --enable-shared \
      --enable-gpl \
      --enable-version3 \
